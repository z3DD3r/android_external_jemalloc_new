...
static size_t accumulate_large_allocs(arena_t* arena) {
  ...
  for (unsigned j = 0; j < SC_NSIZES - SC_NBINS; j++) {
    ...
    uint64_t ndalloc = locked_read_u64(TSDN_NULL, LOCKEDINT_MTX(arena->stats.mtx), &arena->stats.lstats[j].ndalloc);
    uint64_t nmalloc = locked_read_u64(TSDN_NULL, LOCKEDINT_MTX(arena->stats.mtx), &arena->stats.lstats[j].nmalloc);
...
static size_t accumulate_small_allocs(arena_t* arena) {
  ...
  for (unsigned j = 0; j < SC_NBINS; j++) {
    bin = arena_bin_choose(TSDN_NULL, arena, j, &binshard);
...
struct mallinfo je_mallinfo_bin_info(size_t aidx, size_t bidx) {
  ...
  if (aidx < narenas_auto && bidx < SC_NBINS) {
    ...
    if (arena != NULL) {
      unsigned binshard;
      bin_t* bin = arena_bin_choose(TSDN_NULL, arena, (szind_t)bidx, &binshard);
